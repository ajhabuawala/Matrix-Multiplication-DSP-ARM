...
Uint32 Floor( Uint32 i, Uint32 j);
Uint32 Ceil( Uint32 i, Uint32 j);
...
#define SAMPLEMQT_CTRLMSG_SIZE  ZCPYMQT_CTRLMSG_SIZE
    ...
    NORMAL_API DSP_STATUS helloDSP_Execute(IN Uint32 numIterations, Uint8 processorId)
    {
        ...
        Uint16 *mat2;
        Uint32 *result;
        Uint32 finalresult[numIterations][numIterations];       //final result matrix
        ...
        size=size1-size1%16+16;
        
        
        mat1=(Uint16*)malloc(size*size*sizeof(Uint16));
        mat2=(Uint16*)malloc(size*size*sizeof(Uint16));
        result=(Uint32*)malloc(size*size*sizeof(Uint32));
        ...
        if (size==size1)
        {
            for (ii = 0;ii < size; ii++)
            {
              for (jj = 0; jj < size; jj++)
                {
                    mat1[ii*size+jj]=ii+jj*2;
                    mat2[ii*size+jj] =ii+jj*3;
                }
            }
        }
        
        else //expand the matrix then it is easy to do the optimization in DSP side
        {
             for (ii = 0;ii < size; ii++)
             {
                 for (jj = 0; jj < size; jj++)
...
//===============================================how to cut
        ...
        for (i = 1 ; ((numIterations == 0) || (i <= (numIterations + 1))) && (DSP_SUCCEEDED (status)); i++)
        {
            ...
            if(i==3)
            {
                  for(ii=0;ii<size/2;ii++)
                  {
                    for(jj=0;jj<size;jj++)
                    {
                      ...
                      result[ii*size+jj]= msg->data.arg2[jj+ii*size];
                    }
            ...
            if(i==4)
            {
                  ...
                  for(ii=0;ii<size/2;ii++)
                  {
                     for(jj=0;jj<size;jj++)
                       {        
                         result[(size/2+ii)*size+jj]= msg->data.arg2[jj+ii*size];
                       }
                ...
                if (DSP_SUCCEEDED(status))
                {
                            ...
                            for (ii = 0;ii < size; ii++)
                             {
                                 for (jj = 0; jj < size; jj++)
                                 {
                                     msg->data.arg1[jj+ii*size] =mat1[ii*size+jj];
                    ...
                    if(i==2)
                    {
                             for (ii = 0;ii <size; ii++)
                             {
                                 for (jj = 0; jj < size; jj++)
                                 {
                                     msg->data.arg1[jj+ii*size] =mat2[ii*size+jj];
        ...
        for(ii=0;ii<size1;ii++){
            for(jj=0;jj<size1;jj++){
                finalresult[ii][jj] = result[ii*size + jj];
        ...
        //stop and record end time above!!!!!!!!!  result is in finalresult[][] array.
        //============================================================================
        //============================================================================
        //============================================================================
        //============================================================================
        ...
        for(ii=0;ii<size1;ii++)
        {
            ...
            for(jj=0;jj<size1;jj++)
            {
                ...
                SYSTEM_1Print("%u  ", finalresult[ii][jj]);
